...
config = basic.ReadJsonFile(configpath)
...
def main():
    ...
    if not ospath.exists(modsDir):
        mkdir(modsDir)
    basic.WriteJsonFile(f'{datasDir}serverStatus.json', MainServerAPIRequest("mods"))
    mods = basic.ReadJsonFile(f'{datasDir}serverStatus.json')
...
async def ModrinthEachModRequest(projectID: str):
    loop = asyncio.get_event_loop()
    basic.WriteJsonFile(f'{modsDir}{projectID}/project.json', await  loop.run_in_executor(None, ModrinthAPIRequest, f'project/{projectID}'))
...
def MainServerAPIRequest(target: str):
    try:
        return basic.reqget(f"http://{config['serverip']}:8000/items/{target}")
    except basic.reqgeterror:
